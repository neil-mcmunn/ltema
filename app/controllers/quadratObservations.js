/*
 * List screen to view, add, or delete quadrat observations
 *
...
try {
    ...
    var resultRow = db.execute (	'SELECT pa.park_name, ta.transect_name, pl.quadrat_name \
								FROM park pa, transect ta, site_survey ss, quadrat pl \
								WHERE ss.site_id = ta.site_id \
								AND pa.park_id = ss.park_id \
								AND pl.transect_id = ta.transect_id \
								AND pl.quadrat_id = ?', quadratID);
    parkName = resultRow.fieldByName('park_name');
    transectName = resultRow.fieldByName('transect_name');
    quadratName = resultRow.fieldByName('quadrat_name');
} catch (e) {
    var errorMessage = e.message;
    Ti.App.fireEvent("app:dataBaseError", {error: errorMessage});
} finally {
...
var labelText = parkName + ' - ' + transectName + ' - ' + quadratName;
...
$.quadratObservationsWin.setTitleControl(titleLabel);


/* Event Listeners */

Ti.App.addEventListener("app:refreshQuadratObservations", function(e) {
    populateTable();
});
...
$.tbl.addEventListener('click', function(e){
    ...
    if (e.source.toString() == '[object TiUIButton]') {
        var modal = Alloy.createController("quadratObservationsModalDisclosureIcon", {observationID:e.rowData.observationID, title:e.rowData.title, comments:e.rowData.comments, mediaID:e.rowData.mediaID, siteID:siteID}).getView();
        modal.open({
            modal : true,
        ...
        var modal = Alloy.createController("quadratObservationsModal", {observationID:e.rowData.observationID, title:e.rowData.title}).getView();
        modal.open({
            modal : true,
            modalTransitionStyle : Ti.UI.iPhone.MODAL_TRANSITION_STYLE_COVER_VERTICAL,
...
$.tbl.addEventListener('delete', function(e) {
    ...
    try {
        ...
        var observationFiles = db.execute('SELECT med.media_name FROM media med, quadrat_observation pob \
...
$.quadratObservationsWin.addEventListener('close', function(e) {
...
function toggleDoneBtn(){
    ...
    } else if (totalQuadratPercentage >= 400) {
...
function doneBtn(){
    $.quadratObservationsWin.close();
}

//Navigation to addQuadratObservation
function addBtn(){
    //disable add button until screen is returned to focus.  Issue #28
    $.addObservation.enabled = false;

    var addObservation = Alloy.createController("addQuadratObservation", {quadratID: quadratID}).getView();
